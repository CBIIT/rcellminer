...
# 	})
  ...
  # TO DO: REACTIVE LM DATA TABLE OBJECT
  #----[Reactive Variables]--------------------------------------------------------------
  	...
  	for (id in predIds){
  		for (dataType in input$rmPredDataTypes){
  			if (validateEntry(dataType, id, input$rmDataset, srcContentReactive())){
  				xData <- getFeatureData(dataType, id, input$rmDataset, srcContent = srcContentReactive())
  ...
  output$rmTabsetPanel = renderUI({
  	...
  	tabsetPanel(type = "tabs",
  							tabPanel("Data", DT::dataTableOutput("rmData")),
  							tabPanel("Heatmap", d3heatmapOutput("rmHeatmap")),
  							tabPanel("Technical Details", verbatimTextOutput("rmTechDetails")))
  })
  
  #-----[Support for Reactive UI Elements]-----------------------------------------------
  output$rmResponseDataTypeUi <- renderUI({
  	srcContent <- srcContentReactive()
  	selectInput("rmResponseDataType", "Response Data Type",
  ...
  output$rmPredDataTypesUi <- renderUI({
  	srcContent <- srcContentReactive()
  	selectInput("rmPredDataTypes", "Predictor Data Types",
  							choices  = srcContent[[input$rmDataset]][["featurePrefixes"]],
